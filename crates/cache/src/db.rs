...
use crate::automations::{Automation, NewAutomation};
use crate::runs::{Progress, Run, RunResult, RunStatus, TriggerType};
...
impl ControlDb {
    ...
    pub fn mark_interrupted_on_startup(&self) -> Result<u64, CacheError> {
        let count = self
            ...
            .write(|tx| {
                ...
                let now = foundation::util::now_timestamp();
    ...
    pub fn delete_runs_older_than(&self, days: i64) -> Result<u64, CacheError> {
        let count = self
            .store
            .write(|tx| {
                let cutoff = foundation::util::now_timestamp() - (days * 24 * 60 * 60);
    ...
    pub fn record_file_open(&self, path: &str) -> Result<(), CacheError> {
        let now = foundation::util::now_timestamp() as u64;
        self.store
            .write(|tx| {
...
fn default_db_dir() -> PathBuf {
    foundation::dirs::db_dir().unwrap_or_else(|| PathBuf::from("."))
}

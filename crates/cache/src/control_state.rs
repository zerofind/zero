...
impl EraseDeviceId {
    ...
    pub fn from_volume_info(
        ...
        Self {
            ...
            volume_uuid: volume_uuid.map(std::string::ToString::to_string),
            serial_number: serial_number.map(std::string::ToString::to_string),
            size_bytes,
            volume_name: volume_name.map(std::string::ToString::to_string),
            mount_point: mount_point.map(std::string::ToString::to_string),
        }
    }
}
...
impl ControlState {
    ...
    pub fn mark_run_partial(&mut self, id: i64, result: &RunResult, resume_state: Option<&str>) {
        if let Some(r) = self.runs.get_mut(&id) {
            ...
            r.result = Some(result.clone());
            r.resumable = true;
            r.resume_state = resume_state.map(std::string::ToString::to_string);
        }
    }

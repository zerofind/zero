...
use cache::Settings as AutomationSettings;
use cache::{ControlDb, NewAutomation, PathMapping, Triggers};
use foundation::output::{
    AutomationCreateData, AutomationDeleteData, AutomationHistoryData, AutomationInfo,
    AutomationListData, AutomationPath, AutomationRunData, AutomationRunInfo, AutomationRunResult,
    AutomationSettingsData, AutomationShowData, AutomationTriggers, Outputter, Response,
    format_bytes,
};
...
fn cmd_automation_show(out: &Outputter, id: i64) -> anyhow::Result<()> {
    ...
    if out.is_json() {
        let last_run_info = last_run.map(|run| {
            let (files_total, bytes_transferred, errors) =
                run.result.as_ref().map_or((None, None, vec![]), |r| {
                    let total =
...
fn cmd_automation_create(
    ...
    let (resolved_sources, template_excludes): (Vec<PathBuf>, Vec<String>) =
        if let Some(template_id) = &template {
            ...
            if !missing.is_empty() {
                out.error(&format!(
                    "Template '{template_id}' has missing required sources:"
                ));
                for src in &missing {
...
fn cmd_automation_delete(out: &Outputter, id: i64) -> anyhow::Result<()> {
    ...
    cmd_success!(out, "automation delete", duration_ms, data, {
        out.success(&format!("Deleted automation '{name}' (ID {id})"));
    });
    Ok(())
}
...
fn cmd_automation_history(out: &Outputter, id: i64, limit: i64) -> anyhow::Result<()> {
    ...
    for run in &history {
        let status_icon = match run.status {
            ...
            cache::RunStatus::Failed => "✗",
            cache::RunStatus::Partial => "◐",
            cache::RunStatus::Running => "●",
            cache::RunStatus::Cancelled => "○",
        };

...
use dedup::{DedupOptions, DedupResult, delete_duplicates, find_duplicates};
use foundation::output::{
    DupesData, DuplicateFileData, DuplicateGroupData, Outputter, format_bytes, format_duration,
    format_mb,
};
use foundation::{cmd_error, cmd_success};
use search::{FileTypeCategory, IndexManager, SearchIndex, SearchQuery};
...
fn find_duplicates_in_entries(
    ...
    // Build result
    let groups: Vec<dedup::DuplicateGroup> = hash_groups
        .into_iter()
        .filter(|(_, entries)| entries.len() > 1)
        .map(|(hash, entries)| {
            let size = entries.first().map_or(0, |(_, s)| *s);
            let files: Vec<PathBuf> = entries.into_iter().map(|(p, _)| p).collect();
            dedup::DuplicateGroup { hash, size, files }
        })
        .collect();

    let duplicate_count: usize = groups.iter().map(|g| g.files.len() - 1).sum();
    let wasted_bytes: u64 = groups.iter().map(dedup::DuplicateGroup::wasted_bytes).sum();
...
fn output_results(
    ...
    telemetry::record_dedup(result.duplicate_count as u64, bytes_reclaimed.unwrap_or(0));

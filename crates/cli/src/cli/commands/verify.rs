...
use anyhow::Result;
use cache::{CacheEntry, ControlDb, StorageDb};
use differ::{DiffOptions, DiffStatus, diff_with_options, diff_with_progress};
use foundation::output::{
    Outputter, Response, VerifyData, VerifyMismatch, format_bytes, format_count, format_duration,
    format_throughput,
};
use scanner::{ScanOptions, scan_collect};
use std::panic;
use std::path::Path;
...
pub fn cmd_verify_with_cache(
    ...
    let (source_db, dest_db) = match (|| {
        ...
        Ok::<_, cache::CacheError>((source_db, dest_db))
    })() {
        Ok(result) => result,
    ...
    for (rel_path, source_file) in &source_map {
        if let Some(dest_file) = dest_map.get(rel_path) {
            ...
            if let (Ok((sh, s_cached)), Ok((dh, d_cached))) = (source_hash, dest_hash) {
                if s_cached {
                    cache_hits += 1;
                } else {
                    cache_misses += 1;
    ...
    if is_fully_verified {
        out.success(&format!("Verified: {identical} files match"));
...
pub fn get_file_hash_cached(
    ...
    let result =
        hasher::hash_file_with_buffer(&full_path, hasher::HashAlgorithm::Xxh3, hash_buffer)?;

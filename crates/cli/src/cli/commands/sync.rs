...
use foundation::output::{Outputter, Response, SyncData, format_bytes, format_duration};
use progress::AtomicProgress;
use storage::{ListOptions, StorageBackend, StorageEntry};
use sync::{SyncJob, SyncOptions, SyncPhase};
...
fn cmd_sync_local(
    ...
    if out.is_json() {
        ...
        if let Some(tp) = throughput {
            out.kv("Throughput", format!("{tp:.1} MB/s"));
        }
    }
...
async fn storage_sync_impl(
    ...
    if !files_to_copy.is_empty() {
        ...
        for src_entry in &files_to_copy {
            let rel_path = src_entry.path.to_string_lossy().to_string();

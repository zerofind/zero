...
use cache::{CacheEntry, ChecksumBatch, ControlDb, DeviceInfo, MountType};
use disk::DiskInfo;
use foundation::output::{
    Outputter, format_bytes, format_count, format_duration, format_throughput,
};
...
pub fn cmd_index(out: &Outputter, opts: &IndexOptions<'_>) -> anyhow::Result<()> {
            ...
            entries
                ...
                .map(|entry| {
                    ...
                    BUFFER.with(|buf| {
                        ...
                        match hasher::hash_file_with_buffer(&entry.absolute_path, algorithm, &mut buffer) {
    ...
    if stats {
        ...
        out.kv("Hash coverage", format!("{coverage:.1}%"));
    ...
    if let Some(n) = benchmark {
        out.info(&format!("Benchmarking index with {n} files..."));
        out.newline();
        ...
        for file in &files_to_hash {
            ...
            match hasher::hash_file_with_buffer(&full_path, algorithm, &mut hash_buffer) {
                Ok(result) => {

...
use storage::{ListOptions, LocalStorage, StorageBackend};

#[cfg(feature = "opendal")]
use storage::OpenDalStorage;
...
pub fn parse_storage_path(path: &str) -> Result<(Box<dyn StorageBackend + Send + Sync>, String)> {
    ...
    if let Some((scheme, rest)) = path.split_once("://") {
        match scheme {
            ...
            _ => {
                bail!(
                    "Cloud storage scheme '{scheme}' requires the 'opendal' feature.\n\
...
async fn download_directory(
    ...
    let total_bytes: u64 = entries.iter().map(storage::StorageEntry::size).sum();

...
pub fn cmd_telemetry(out: &Outputter, cmd: &TelemetryCommands) -> anyhow::Result<()> {
    match cmd {
        TelemetryCommands::Status => {
            let enabled = zero::telemetry::is_enabled();
            if enabled {
        ...
        TelemetryCommands::Show => {
            if let Some(payload) = zero::telemetry::show_payload() {
                out.println(&payload);
            } else {

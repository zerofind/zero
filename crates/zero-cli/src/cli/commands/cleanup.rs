...
use zero::cleanup::{DevScanProgress, scan_dev_garbage};
...
fn cmd_cleanup_dev(
    ...
    if !out.is_json() {
        ...
        out.println(&format!(
            "Scanning {} for developer build artifacts...",
            display
        ));
        out.newline();
    }
    ...
    // Display results
    if out.is_json() {
        let response = zero::output::Response::success("cleanup", duration_ms, &scan_result);
        out.result(&response);
        if dry_run || !all {
            return Ok(());
        }

...
use zero::cache::ControlDb;
use zero::disk::VolumeInfo;
use zero::disk::erase::platform;
use zero::disk::erase::state::DeviceIdentifier;
use zero::disk::erase::{
...
pub fn cmd_erase(out: &Outputter, args: &EraseArgs) -> anyhow::Result<()> {
                ...
                EraseState::find_by_volume_name_or_mount_point(&control_db, &path_str)
            {
                ...
                match get_device_info(&device_path) {
                    Ok(device_info) => {
                        ...
                        if !out.is_json() {
                            out.info(&format!(
                                "Volume '{}' is not mounted - resuming interrupted erase...",
                                path_str
                            ));
                            out.println(&format!(
                                "  Device:      {} ({})",
    ...
    let target = match prepare_erase(&volume) {
        ...
        Err(EraseError::SystemDisk(name)) => {
            let msg = format!(
                "SAFETY BLOCK: '{}' is an internal/system disk. \
...
fn run_wipe_with_resume_internal<F>(
    _access: &mut dyn zero::disk::erase::platform::StorageAccess,
    target: &EraseTarget,
    scheme: &zero::disk::erase::Scheme,
    options: &EraseOptions,
    resume_state: Option<EraseState>,
    progress_callback: F,
) -> Result<zero::disk::erase::wipe::EraseResult, EraseError>
where
    F: FnMut(EraseProgress),
{
    ...
    zero::disk::erase::wipe::run_wipe_with_resume(
        target,
        scheme,
        options,
        resume_state,

...
use zero::output::*;
use zero::todo::{Task, TaskStatus, TodoManager};
...
pub fn cmd_todo(out: &Outputter, cmd: Option<&TodoCommands>) -> anyhow::Result<()> {
    match cmd {
        ...
        Some(TodoCommands::View(args)) => cmd_view(out, args.first().map(|s| s.as_str())),
    }
}
...
fn cmd_toggle(out: &Outputter, ids: &[u64]) -> anyhow::Result<()> {
    ...
    cmd_success!(out, "todo.toggle", 0u64, data, {
        for (id, was_open) in &toggled {
            if *was_open {
                out.success(&format!("#{} → done", id));
            } else {
                out.success(&format!("#{} → open", id));
            }
        }
...
fn cmd_move(out: &Outputter, id: u64, after: &str, list: Option<&str>) -> anyhow::Result<()> {
    ...
    let after_id = match after.to_lowercase().as_str() {
        ...
        s => Some(s.parse::<u64>().map_err(|_| {
            anyhow::anyhow!("Invalid position '{}'. Use task ID, 'top', or 'last'", s)
        })?),
    };
    ...
    cmd_success!(out, "todo.move", 0u64, data, {
        match (list, after_id) {
            (Some(l), None) => out.success(&format!("Moved #{} to top of {}", id, l)),
            (Some(l), Some(aid)) => out.success(&format!("Moved #{} after #{} in {}", id, aid, l)),
            (None, None) => out.success(&format!("Moved #{} to top", id)),
            (None, Some(aid)) => out.success(&format!("Moved #{} after #{}", id, aid)),
        }
    });
    Ok(())
}

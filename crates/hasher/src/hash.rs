...
fn hash_blake3(
    ...
    if file_size > MMAP_THRESHOLD {
        // SAFETY: Read-only memory mapping of a file opened read-only.
        // If another process truncates the file after mapping, accessing
        // unmapped pages causes SIGBUS. This is acceptable: Zero hashes
        // user files that are not expected to be concurrently truncated.
        // A wrong hash from concurrent writes is detected on the next sync.
        let mmap = unsafe {
...
fn hash_blake3_with_atomic_progress(
    ...
    if file_size > MMAP_THRESHOLD {
        // SAFETY: Read-only mapping of a read-only file. See hash_blake3 for
        // full rationale on concurrent-modification risk.
        let mmap = unsafe {
            memmap2::Mmap::map(file).map_err(|e| HashError::ReadError {
                path: path_str.to_string(),
                source: e,

...
//! - `file_types_base.toml` - File type classifications by extension
//! - `cleanup_*.toml` - Cleanup category definitions
...
/// Root structure for `file_types_base.toml`
...
impl MatchPatterns {
    ...
    pub fn for_current_os(&self) -> Vec<&str> {
        match self {
            MatchPatterns::Universal(patterns) => {
                patterns.iter().map(std::string::String::as_str).collect()
            }
            MatchPatterns::PerOs { macos, .. } => macos
                .as_ref()
                .map(|p| p.iter().map(std::string::String::as_str).collect())
                .unwrap_or_default(),
    ...
    pub fn is_empty(&self) -> bool {
        match self {
            ...
            MatchPatterns::PerOs {
                ...
                macos.as_ref().is_none_or(std::vec::Vec::is_empty)
                    && linux.as_ref().is_none_or(std::vec::Vec::is_empty)
                    && windows.as_ref().is_none_or(std::vec::Vec::is_empty)
            }
...
impl Os {
    ...
    /// Get the OS identifier string (used in profile filenames like `cleanup_macos.toml`)

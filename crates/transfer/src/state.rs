...
impl TransferTx<'_> {
    ...
    ///
    /// Non-UTF-8 paths are skipped — they cannot be reliably keyed in the
    /// completion log and will re-transfer on resume (safe default).
    pub fn append_entry(&mut self, entry: &CompletedEntry) {
        let Some(path_str) = entry.path.to_str() else {
            tracing::warn!(path = %entry.path.display(), "non-UTF-8 path, skipping completion log (will re-transfer on resume)");
            return;
        };
        let key = path_str.as_bytes().to_vec();
        let value =

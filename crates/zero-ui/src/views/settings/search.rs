...
impl SettingsView {
                                ...
                                h_flex()
                                    ...
                                    .child(
                                        Button::new("reindex-all")
                                            ...
                                            .small()
                                            .ghost()
                                            .disabled(self.reindexing_all || self.settings.search_roots.is_empty())
                        ...
                        v_flex()
                            ...
                            .children(
                                self.settings
                                    ...
                                    .map(|(i, root)| {
                                        let is_last =
                                            i == self.settings.search_roots.len() - 1;
                                        let is_reindexing = self.reindexing_root == Some(i);

                                        // Overlap detection
                                        let overlap = self.settings.search_roots.iter().enumerate().find_map(|(j, other)| {
                                            if j == i { return None; }
                                            let root_str = root.to_string_lossy();
                                            let other_str = other.to_string_lossy();
                                            let root_with_sep = format!("{}/", root_str.trim_end_matches('/'));
                                            let other_with_sep = format!("{}/", other_str.trim_end_matches('/'));
    ...
    pub(super) fn add_root(&mut self, path: PathBuf, cx: &mut Context<Self>) {
        ...
        self.search.update(cx, |svc, cx| svc.add_root(&path_str, cx));
    ...
    pub(super) fn remove_root(&mut self, idx: usize, cx: &mut Context<Self>) {
        ...
        let path_str = path.to_string_lossy().to_string();
        self.search.update(cx, |svc, cx| svc.remove_root(&path_str, cx));
        self.indexed_file_count = self.search.read(cx).file_count();

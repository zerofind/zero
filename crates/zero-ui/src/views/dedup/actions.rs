...
use zero::dedup::{DedupProgress, DedupStreamEvent};
    ...
    pub(super) fn start_scan(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        cx.background_executor()
            .spawn(async move {
                ...
                if let Err(e) =
                    zero::dedup::find_duplicates_streaming(&path, options, progress_bg, tx, mgr_ref)
                {
...
fn filter_to_options(filter: DedupFilter) -> zero::dedup::DedupOptions {
    match filter {

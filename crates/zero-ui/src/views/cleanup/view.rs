...
impl CleanupView {
    ...
    pub(super) fn start_scan(&mut self, cx: &mut Context<Self>) {
        ...
        cx.spawn(async move |this, cx| {
            ...
            let active_groups: Vec<CleanupGroup> = zero::prelude::load_cleanup()
            ...
            for group in active_groups {
                ...
                let summary = cx
                    .background_executor()
                    .spawn(async move { zero::cleanup::execute_group_cleanup(&mgr, group).ok() })
                    .await;
        ...
        cx.spawn(async move |this, cx| {
            let summary = if let Some(s) = cached {
                ...
                cx.background_executor()
                    .spawn(async move {
                        zero::cleanup::execute_group_cleanup(&manager, cleanup_group).ok()
                    })
                    .await
            };
...
fn group_dev_results(summary: &zero::cleanup::GroupSummary) -> Vec<DevDirEntry> {

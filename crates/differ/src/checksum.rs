...
use hasher::{HashAlgorithm, hash_file_with_atomic_progress, hash_file_with_buffer};
use progress::AtomicProgress;
use scanner::FileEntry;

use super::compare::{ComputedHash, DiffOptions, DiffProgress, HashSender};
...
pub(crate) struct ChecksumDiffConfig<'a> {
    ...
    pub source_hash_tx: Option<HashSender>,
    pub dest_hash_tx: Option<HashSender>,
}
    ...
    F: Fn(DiffProgress) + Send + Sync,
{
    ...
    // MTx is Clone, no Arc needed
    let source_tx = source_hash_tx;
...
fn send_or_collect_hash(
    ...
    if let Some(h) = hash {
        let sent = tx.as_ref().is_some_and(|tx| tx.send(h.clone()).is_ok());

...
impl ZeroAgent {
    ...
    pub fn ask(
        &mut self,
        question: &str,
    ) -> crossfire::AsyncRx<crossfire::mpsc::List<StreamEvent>> {
        let (tx, rx) = crossfire::mpsc::unbounded_async::<StreamEvent>();
...
async fn run_agentic_loop(
    ...
    thinking: bool,
    thinking_budget: u32,
    tx: &crossfire::MTx<crossfire::mpsc::List<StreamEvent>>,
) -> Result<String> {
    ...
    let tool_count = provider_tools.as_ref().map_or(0, std::vec::Vec::len);
    info!(
            ...
            match part {
                ...
                LanguageModelStreamPart::Error(e) => {
                    let raw = e
                        .error
                        .as_str()
                        .map_or_else(|| e.error.to_string(), String::from);
            ...
            match output {
                ...
                Some(ToolOutput::Error(error)) => {
                    let error_msg = error
                        .error
                        .as_str()
                        .map_or_else(|| error.error.to_string(), String::from);

...
/// Status of a completed (or in-progress) sync run — mapped from `cache::runs::RunStatus`.
...
/// A single sync history entry — built from `cache::runs::Run`.
#[derive(Debug, Clone)]
struct RunInfo {
...
/// An automation that targets a specific drive — built from `cache::automations::Automation`.
#[derive(Debug, Clone)]
...
impl DrivesPopover {
        ...
        for (auto_id, auto_name) in &name_by_id {
            ...
            for run in runs {
                let status = match run.status {
                    cache::runs::RunStatus::Success => RunStatus::Success,
                    cache::runs::RunStatus::Failed => RunStatus::Failed,
                    cache::runs::RunStatus::Running => RunStatus::Running,
                    cache::runs::RunStatus::Partial | cache::runs::RunStatus::Cancelled => {
                        RunStatus::Warning
                    }
                };
    ...
    fn discover_drives() -> Vec<DriveInfo> {
        ...
        if let Some(home) = dirs::home_dir()
            && let Ok(vol) = disk::VolumeInfo::for_path(&home)
        {
        ...
        if let Ok(all) = disk::VolumeInfo::all() {
            for vol in all {
                if !vol.is_external() {
                    continue;
                }

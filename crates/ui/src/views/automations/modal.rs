...
impl AutomationModal {
    ...
    fn build_new_automation(
        ...
        name: String,
        dest: String,
    ) -> cache::automations::NewAutomation {
        let triggers = cache::automations::Triggers {
            on_mount: self.on_mount,
            on_change: self.on_change,
            on_schedule: None,
        };

        let paths: Vec<cache::automations::PathMapping> = self
            .sources
            .iter()
            .map(|s| cache::automations::PathMapping {
                source: s.clone(),
                dest: String::new(),
                exclude: Vec::new(),
            })
        ...
        let settings = cache::automations::Settings {
            verify: self.verify,
    ...
    async fn save_to_db(
        ...
        let db = cache::CacheDb::open().map_err(|e| format!("Database error: {e}"))?;

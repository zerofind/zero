...
use disk::erase::state::EraseState as PersistentEraseState;
...
impl SecureEraseView {
    ...
    pub(super) fn select_volume(&mut self, idx: usize, cx: &mut Context<Self>) {
        ...
        if let Some(vol) = self.volumes.get(idx).cloned() {
            ...
            self.resume_info = cache::ControlDb::open()
                .ok()
                .and_then(|db| {
    ...
    fn run_erase(
        ...
        let volume = disk::VolumeInfo::for_path(&mount_point)
            .map_err(|e| format!("Failed to get volume info: {e}"))?;

        let _target =
            disk::erase::prepare_erase(&volume).map_err(|e| format!("Cannot erase: {e}"))?;

        let level = match level_idx {
            0 => disk::erase::EraseLevel::Quick,
            2 => disk::erase::EraseLevel::Strong,
            _ => disk::erase::EraseLevel::Normal,
        };

        let options = disk::erase::EraseOptions::new(level);
        let scheme = level.scheme();

        let result = disk::erase::wipe::run_wipe_cancellable(
            &_target,
            &scheme,
            &options,
            resume_state,

...
use search::{IndexManager, SearchIndex, SearchResult, hash_path};
use search::{load_index_snapshot, persistence};
            ...
            for root in &roots {
                ...
                let build_result = cx
                    ...
                    .spawn(async move {
                        ...
                        Ok::<_, search::IndexError>((root_clone, index, count))
                    })
                    .await;
        ...
        cx.spawn(async move |this, cx| {
            ...
            let build_result = cx
                ...
                .spawn(async move {
                    ...
                    let snapshot = indexes_dir.join(format!("{hash}.zidx"));
                    persistence::save_index(&index, &snapshot)?;
                    Ok::<_, search::IndexError>((path_owned, index, count))
                })
                .await;
        ...
        cx.spawn(async move |this, cx| {
                    ...
                    match load_result {
                        ...
                        Err(e) => {
                            ...
                            let is_not_found = matches!(&e,
                                search::IndexError::Io(io) if io.kind() == std::io::ErrorKind::NotFound
                            );
                            if is_not_found {
            ...
            let build_result = cx
                ...
                .spawn(async move {
                    ...
                    let snapshot = indexes_dir.join(format!("{hash}.zidx"));
                    persistence::save_index(&index, &snapshot)?;
                    Ok::<_, search::IndexError>((home_clone, index, count))
                })
                .await;

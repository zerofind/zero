...
use code::CodeIndex;
use mcp::{McpConfig, McpHandle, generate_api_key};
...
impl McpService {
    pub fn new(_cx: &mut Context<Self>) -> Self {
        Self {
            ...
            running: false,
            port: 45557,
            api_key: mcp::auth::load_or_create_api_key(),
        }
    }
    ...
    pub fn start(&mut self, manager: IndexManager, port: u16, cx: &mut Context<Self>) {
        ...
        let handle = mcp::start_server(config, manager, code);
    ...
    pub fn regenerate_api_key(&mut self) {
        ...
        // Persist the new key
        if let Some(data_dir) = foundation::dirs::data_dir() {

...
use hasher::{HashAlgorithm, hash_file_prefix, hash_file_with_buffer};
use scanner::{CrawlProgress, ScanOptions, scan_with_progress};
...
/// Sender type for dedup stream channels
pub type StreamSender = crossfire::MTx<crossfire::mpsc::List<DedupStreamEvent>>;
...
pub fn find_duplicates_streaming(
    ...
    let total_potential: usize = potential_duplicates.iter().map(std::vec::Vec::len).sum();
...
fn emit_size_only_groups(groups: Vec<Vec<CandidateFile>>, tx: &StreamSender) {
    for entries in groups {
        let size = entries.first().map_or(0, |e| e.size);
...
fn hash_groups_streaming(
    ...
    for group in groups {
        ...
        let file_size = group.first().map_or(0, |e| e.size);

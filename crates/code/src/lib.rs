...
impl CodeIndex {
    ...
    pub fn new() -> Result<Self, CodeIndexError> {
        let index_dir = foundation::dirs::code_dir().ok_or_else(|| {
    ...
    pub fn index_project(&mut self, path: &Path) -> Result<&CodeProject, CodeIndexError> {
        ...
        // Persist .cidx
        let cidx_path = self.index_dir.join(format!("{hash}.cidx"));
    ...
    pub fn project_symbols(
        ...
        let project_path = self.projects.get(&hash).map_or_else(
            || canonical.display().to_string(),
            |p| p.path.display().to_string(),
        );
    ...
    pub fn overview(&mut self, project: &Path) -> Result<Option<ProjectOverview>, CodeIndexError> {
        ...
        for (file_path, elements) in &by_file {
            ...
            let module = Path::new(file_path).parent().map_or_else(
                || ".".to_string(),
                |p| {

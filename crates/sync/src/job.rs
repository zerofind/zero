...
use differ::{ComputedHash, DiffOptions, DiffStatus, diff_with_atomic_progress, diff_with_options};
use progress::{AtomicProgress, ProgressDisplay};
use scanner::ScanFilter;
use scanner::{FileEntry, ScanOptions};
use transfer::{CopyOptions, sync_dir_permissions};
            ...
            if options.verify {
                ...
                let control = ControlDb::open().map_err(|e| {
                    SyncError::IoError(std::io::Error::other(format!(
                        "Failed to open control database: {e}"
                    )))
                })?;
                ...
                let source_db = control.open_storage_db(&source_storage).map_err(|e| {
                    SyncError::IoError(std::io::Error::other(format!(
                        "Failed to open source storage database: {e}"
                    )))
                })?;

                let dest_db = control.open_storage_db(&dest_storage).map_err(|e| {
                    SyncError::IoError(std::io::Error::other(format!(
                        "Failed to open dest storage database: {e}"
                    )))
                })?;
    ...
    fn run_internal_with_callback(
        ...
        let (source_files, dest_files) = if self.options.use_checksum {
                ...
                (&self.source_storage_db, &self.dest_storage_db)
            {
                ...
                eprintln!(" {src_cache_hits} source, {dest_cache_hits} dest cached");
        ...
        let diff_result = if self.options.use_checksum {
            ...
            let (source_hash_tx, source_hash_rx) = cf_mpsc::unbounded_blocking::<ComputedHash>();
            let (dest_hash_tx, dest_hash_rx) = cf_mpsc::unbounded_blocking::<ComputedHash>();
            ...
            if total_cached > 0 {
                eprintln!("Cached {total_cached} checksums incrementally");
            }
        ...
        let files_to_transfer: Vec<&FileEntry> = source_files
            ...
            .filter(|f| {
                diff_entry_map
                    .get(f.path.as_path())
                    .is_some_and(|status| status.needs_transfer())
            })
            .collect();
        ...
        let files_to_copy: Vec<&FileEntry> = files_to_transfer.clone();
        ...
        let identical_bytes: u64 = source_files
            ...
            .filter(|f| {
                diff_entry_map
                    .get(f.path.as_path())
                    .is_some_and(|status| **status == DiffStatus::Identical)
            })
            .map(|f| f.size)
            .sum();
        ...
        if self.options.dry_run {
            return Ok(crate::phases::phase_dry_run(
                &files_to_copy,
                &files_to_delete,
        ...
        let transfer_ctx = crate::phases::TransferContext {
            dest: &self.dest,
            copy_options,
        ...
        {
            crate::phases::phase_cache_flush(
                control_db,
                source_db,
                dest_db,
                source_id,
    ...
    fn cleanup_empty_dirs(&self, dir: &Path) -> Result<(), SyncError> {
        ...
        for entry in fs::read_dir(dir)? {
            ...
            if path.is_dir() {
                ...
                if path.file_name().is_some_and(|n| n == ".zero") {
                    continue;
                }

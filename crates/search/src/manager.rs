...
use cache::{ControlDb, IndexedRoot};
...
impl IndexManager {
    ...
    pub fn remove_root(&mut self, root: &str) -> usize {
        ...
        let file_count = self
            .indexes
            .get(root)
            .map_or(0, super::search::SearchIndex::file_count);
    ...
    pub fn total_file_count(&self) -> usize {
        self.roots_cache
            ...
            .map(|(root, cached)| {
                self.indexes
                    .get(root)
                    .map_or(cached.file_count, super::search::SearchIndex::file_count)
            })
            .sum()
    }
    ...
    pub fn total_dir_count(&self) -> usize {
        self.roots_cache
            ...
            .map(|(root, cached)| {
                self.indexes
                    .get(root)
                    .map_or(cached.dir_count, super::search::SearchIndex::dir_count)
            })
            .sum()
    }
    ...
    pub fn total_bytes(&self) -> u64 {
        self.roots_cache
            ...
            .map(|(root, cached)| {
                self.indexes
                    .get(root)
                    .map_or(cached.total_bytes, super::search::SearchIndex::total_bytes)
            })
            .sum()
    }
    ...
    pub fn iter_all(&self) -> impl Iterator<Item = FileNode> + '_ {
        self.indexes
            .values()
            .flat_map(super::search::SearchIndex::iter)
    }

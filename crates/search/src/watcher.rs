...
//! use std::sync::Arc;
//! use std::path::Path;
//! use search::{SearchIndex, IndexWatcher};
//!
//! // Build initial index
    ...
    fn apply_event(&mut self, event: &FileEvent) {
        ...
        match event.kind {
            ...
            FileChangeKind::Modified => {
                for path in &event.paths {
                    let Some(path_str) = path.to_str() else {
                        tracing::warn!(path = %path.display(), "non-UTF-8 path, skipping index update");
                        continue;
                    };
            ...
            FileChangeKind::Deleted => {
                for path in &event.paths {
                    let Some(path_str) = path.to_str() else {
                        tracing::warn!(path = %path.display(), "non-UTF-8 path, skipping index removal");
                        continue;
                    };

...
impl CompactNameIndex {
    ...
    fn build_from(map: &BTreeMap<String, Vec<u32>>) -> Self {
        let total_names_bytes: usize = map.keys().map(std::string::String::len).sum();
        let total_indices: usize = map.values().map(std::vec::Vec::len).sum();
...
impl SearchIndex {
    ...
    pub fn root(&self) -> Option<&str> {
        self.roots.first().map(std::string::String::as_str)
    }
    ...
    pub fn add_root_with_progress(
        ...
        let Some(root_str) = root.to_str() else {
            tracing::warn!(root = %root.display(), "non-UTF-8 root path, skipping");
            return Ok(());
        };
        ...
        for result in walk_dir {
            ...
            match result {
                Ok(entry) => {
                    ...
                    // Store ABSOLUTE path — skip non-UTF-8 paths
                    let Some(abs_path) = entry_path.to_str().map(String::from) else {
                        tracing::warn!(path = %entry_path.display(), "non-UTF-8 path, skipping");
                        continue;
                    };
    ...
    pub fn insert(&mut self, node: FileNode) -> bool {
        ...
        let extension = node.extension().map(std::string::ToString::to_string);
        let mtime = node.mtime;

        // Push path into arena first — if it doesn't fit, skip entirely
        let Some((path_offset, path_len)) = self.path_arena.push(&node.path) else {
            return false;
        };

        let index = self.slab.len();

        // Update stats only after successful arena push

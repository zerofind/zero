...
pub fn phase_transfer(ctx: &TransferContext<'_>, files_to_copy: &[&FileEntry]) -> TransferResult {
    ...
    let copy_file_fn = |file: &&FileEntry| {
        ...
        let copy_result = if use_chunked {
            ...
            .map_err(|e| crate::transfer::CopyError::CopyError {
        ...
        match copy_result {
            ...
            Err(e) => {
                ...
                if consec >= 50 {
                    ...
                    eprintln!(
                        "\r\n⚠ Too many consecutive errors ({}) - source may be disconnected. Aborting...\n",
                        consec
                    );
                }
            }
    ...
    if total_files_to_copy > 0 {
        ...
        if was_aborted {
            eprintln!(
                "\r[{}/{}] ABORTED ({:.1}/{:.1} {}) - {} errors                    ",
                final_done, grand_total_files, final_size, total_size, unit, final_errors
            );
        } else if final_errors > 0 {
            eprintln!(
                "\r[{}/{}] ({:.1}/{:.1} {}) - Completed with {} errors              ",
                final_done, grand_total_files, final_size, total_size, unit, final_errors
            );
        } else {
            eprintln!(
                "\r[{}/{}] 100% ({:.1}/{:.1} {}) - Done!                              ",
                final_done, grand_total_files, final_size, total_size, unit
            );
        }
    }
...
pub fn phase_cache_flush(
    ...
    let cache_result: Result<(), ()> = {
        ...
        for (path, size, mtime, hash_bytes) in hashes.iter() {
            let source_entry =

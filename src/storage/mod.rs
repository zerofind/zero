...
//!
//! // Cloud storage (with opendal feature)
...
/// Non-opendal version - only supports local storage
...
pub fn parse_storage_url(
    ...
    if url.contains("://") && !url.starts_with("file://") {
        return Err(StorageError::UnsupportedScheme {
            ...
            hint: "Enable the 'opendal' feature for cloud storage support".to_string(),
        });
    }
...
mod tests {
    ...
    #[cfg(not(feature = "opendal"))]
    #[test]
    fn test_unsupported_scheme_without_opendal() {

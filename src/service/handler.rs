...
impl ServiceHandler {
    ...
    pub async fn new(db: CacheDb, logger: ServiceLogger) -> Result<Self, crate::cache::CacheError> {
        let db = Arc::new(db);
    ...
    pub async fn load_index(&self) -> Result<u64, String> {
        ...
        *guard = Some(manager);
        ...
        self.logger.info(
            "handler",
            &format!("Search index loaded: {} files", file_count),
        );
    ...
    async fn handle_search_index(&self, request: &JsonRpcRequest) -> Result<Value, JsonRpcError> {
        ...
        *guard = Some(manager);
        ...
        self.logger.info(
            "handler",
            &format!("Index built: {} files, {} bytes", file_count, total_bytes),
        );
    ...
    async fn handle_drives(&self, _request: &JsonRpcRequest) -> Result<Value, JsonRpcError> {
        use crate::disk::{DiskInfo, VolumeInfo};
        use std::path::Path;
    ...
    async fn handle_automation_create(
        ...
        use crate::cache::{NewAutomation, PathMapping, Settings, Triggers};
        ...
        let runs: Vec<Value> = history
            ...
            .map(|r| {
                ...
                serde_json::json!({
                    ...
                    "trigger": r.trigger.as_ref().map(|t| t.as_str()),

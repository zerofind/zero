...
//!
//! ```ignore
//! use zero::cleanup::profile_query::{ProfileCleanupQuery, execute_cleanup_query};
//! use zero::profiles::{load_cleanup, CleanupGroup};
//!
...
use crate::profiles::{CleanupGroup, ResolvedCleanupCategory, load_cleanup};
...
impl<'a> ProfileCleanupQuery<'a> {
    ...
    pub fn with_min_age_days(mut self, days: u32) -> Self {
        self.min_age_override = Some(days as u64 * 24 * 60 * 60);
        self
    }
    ...
    fn is_excluded(&self, path: &str) -> bool {
        for exclude in &self.category.exclude {
            ...
            let expanded = if exclude.starts_with("~/") {
                dirs::home_dir()
                    .map(|h| h.join(&exclude[2..]).to_string_lossy().to_string())
...
pub fn execute_group_cleanup(
    manager: &IndexManager,
    group: CleanupGroup,
) -> Result<GroupSummary, crate::profiles::ProfileError> {
    let profile = load_cleanup()?;
...
pub fn execute_full_cleanup_scan(
    manager: &IndexManager,
) -> Result<ProfileCleanupSummary, crate::profiles::ProfileError> {
    let profile = load_cleanup()?;
...
pub fn execute_category_cleanup(
    manager: &IndexManager,
    category_id: &str,
) -> Result<Option<ProfileCleanupResult>, crate::profiles::ProfileError> {
    let profile = load_cleanup()?;

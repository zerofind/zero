...
//! 1. Profile configuration (`profiles/file_types.toml`) - preferred
...
impl TypeIndex {
    ...
    fn try_load_from_profile(&mut self) -> bool {
        match crate::profiles::load_file_types() {
            Ok(profile) => {
    ...
    fn build_extension_map_from_profile(&mut self, profile: &crate::profiles::FileTypesProfile) {
    ...
    pub fn count(&self, category: FileTypeCategory) -> u64 {
        self.bitmaps.get(&category).map(|b| b.len()).unwrap_or(0)
    }
    ...
    pub fn memory_usage(&self) -> usize {
        let category_size: usize = self.bitmaps.values().map(|b| b.serialized_size()).sum();
...
mod tests {
    ...
    fn test_from_profile_explicit() {
        let profile = crate::profiles::load_file_types().expect("should load profile");

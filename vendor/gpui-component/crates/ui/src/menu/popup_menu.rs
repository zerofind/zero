...
use gpui::{
    ...
    WeakEntity, Window, anchored, canvas, div, prelude::FluentBuilder, px,
};
...
impl PopupMenu {
    ...
    fn update_submenu_menu_anchor(&mut self, window: &Window) {
        ...
        let (anchor, left) = if max_width + bounds.origin.x > window.bounds().size.width {
            ...
            (Corner::TopLeft, bounds.size.width - px(12.))
        };
    ...
    fn render_item(
        ...
        // Separators are plain divs — no MenuItemElement wrapper
        if matches!(item, PopupMenuItem::Separator) {
            return div()
                .h(px(1.))
                .mx(px(-6.))
                .my(px(4.))
                .bg(cx.theme().popover_border)
                .into_any_element();
        }
        ...
        let element = match item {
            PopupMenuItem::Separator => unreachable!(),
                        ...
                        anchored()
                            ...
                            .child(
                                div()
                                    ...
                                    .when(!is_bottom_pos, |this| this.top(px(-6.)))
                                    .left(left)
                                    .child(menu.clone()),
                            )
